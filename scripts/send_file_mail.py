...
ap.add_argument("-e", "--path", required = True, help="path", nargs='+')
ap.add_argument("-i", "--content", required = True, help="content", nargs='+')
args = vars(ap.parse_args())

######### Setup your stuff here #######################################

attachments = ["/nfs_sunshine/suivi_stock_mail_" + str(args["date"]) + ".csv"]
...
receivers = ["youssouf.diakite@orange.com", "abdoulaziz.gbane@orange.com"]
toaddr  = ", ".join(receivers)
replyto = fromaddr # unless you want a different reply-to

msgsubject = "SUIVI STOCK MAIL " + str(args["date"])
...
print " *********** REMOVE DATA /nfs_sunshine/suivi_stock_mail_" + str(args["date"]) + " ***********"

#commande =  "rm  -rf /nfs_sunshine/suivi_stock_mail_" + str(args["date"])
